...
class OptitrackIO:
    ...
    def _parse_folder(self):
        ...
        # Get time relative to 0
        time = np.arange(len(postime)) * (1 / sampling_rate)
    ...
    def to_position(self, t_start=0):
        return Position.init_from_xyz_arrays(self.x, self.y, self.z, t_start=t_start, sampling_rate=self.sampling_rate)
